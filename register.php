...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($full_name) || empty($email) || empty($phone) || empty($password) || empty($confirm_password) || empty($address)) {
        $error = 'Please complete all required fields including your full residential address.';
        ...
        if ($stmt->fetch()) {
            ...
            $insert = $db->prepare("INSERT INTO users (role_id, full_name, email, phone, password_hash, address, municipality, district, is_verified, is_active, created_at) 
                                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1, 1, NOW())");
            $insert->execute([$role_id, $full_name, $email, $phone, $passwordHash, $address, $municipality, $district]);
        ...
        <div class="col-lg-8 col-md-10">
            ...
            <div class="card p-3 mb-4 bg-light border-0 shadow-sm rounded-4 text-center">
                <div class="d-flex justify-content-center gap-3">
                    <a href="register.php" class="btn btn-primary fw-bold px-4 py-2 shadow-sm"><i class="fa-solid fa-user me-2"></i>Citizen Registration</a>
                    <a href="vendor/register.php" class="btn btn-outline-success fw-bold px-4 py-2"><i class="fa-solid fa-shop me-2"></i>Vendor / Business Registration</a>
                </div>
            </div>
            ...
            <div class="card card-custom p-4 shadow-lg border-0">
                <div class="text-center mb-4">
                    ...
                    <p class="text-muted small">Create your account to access government services, browse local markets, and place orders.</p>
                </div>
                ...
                <form method="POST" action="register.php">
                    <div class="row g-3 mb-3">
                        ...
                        <div class="col-md-6">
                            <label class="form-label fw-semibold">Full Residential Address *</label>
                            <input type="text" name="address" class="form-control" placeholder="e.g. Boudha, Kathmandu or Ward 10, New Baneshwor" value="<?= sanitize($_POST['address'] ?? '') ?>" required>
                        </div>
                        <div class="col-md-6">
